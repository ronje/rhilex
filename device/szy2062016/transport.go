...
type SZY206SerialTransporter struct {
	port         typex.GenericRWC
	readTimeout  time.Duration
	writeTimeout time.Duration
}

func NewSZY206SerialTransporter(config TransporterConfig) *SZY206SerialTransporter {
	return &SZY206SerialTransporter{
...
func (dlt *SZY206SerialTransporter) SendFrame(aduRequest []byte) ([]byte, error) {
 ...
 *
 */
func (dlt *SZY206SerialTransporter) ReadFrame(rwc typex.GenericRWC) (aduResponse []byte, err error) {
	// 0x68
	var start1 byte
	for {
		var b byte
