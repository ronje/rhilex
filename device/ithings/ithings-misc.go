...
func GetLoginDevice(userName string) (*LoginDevice, error) {
	...
	if len(keys) != 4 {
		return nil, fmt.Errorf("userName not right")
	}
...
func GetClientIDInfo(ClientID string) (*LoginDevice, error) {
	...
	case 3:
		if clientIDs[0] != "register" {
			return nil, fmt.Errorf("clientID not right")
		}
		lg := &LoginDevice{
	...
	default:
		...
		if len(ClientID) < ProductIdLen {
			return nil, fmt.Errorf("clientID length not enough")
		}
		lg := &LoginDevice{
...
func NewPwdInfo(signature string, signMethod string) (*PwdInfo, error) {
	...
	default:
		return nil, fmt.Errorf("password not support encrypt method:" + signMethod)
	}
	return &PwdInfo{
		token:      signature,
		hmac:       signMethod,
...
func (p *PwdInfo) CmpPwd(signature, secret string) error {
	...
	password := p.HmacHandle(signature, pwd)
	if password != p.token {
