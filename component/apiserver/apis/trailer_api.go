...
*
 */
func StopGoods(c *gin.Context, ruleEngine typex.Rhilex) {
...
*
 */
func GoodsDetail(c *gin.Context, ruleEngine typex.Rhilex) {
	uuid, _ := c.GetQuery("uuid")
...
func GoodsList(c *gin.Context, ruleEngine typex.Rhilex) {
	data := []goodsVo{}
...
*
 */
func DeleteGoods(c *gin.Context, ruleEngine typex.Rhilex) {
	uuid, _ := c.GetQuery("uuid")
	...
	if goods := trailer.Get(uuid); goods != nil {
		trailer.RemoveBy(goods.Info.UUID, "RHILEX")
	}
	// 数据库和内存都要删除
...
*
 */
func CleanGoodsUpload(c *gin.Context, ruleEngine typex.Rhilex) {
	// 清理进程包
...
*
 */
func CreateGoods(c *gin.Context, ruleEngine typex.Rhilex) {
...
*
 */
func UpdateGoods(c *gin.Context, ruleEngine typex.Rhilex) {
	fileHeader, err := c.FormFile("file")
	if err != nil {
		c.JSON(common.HTTP_OK, common.Error400(err))
		return
...
*
 */
func StartGoods(c *gin.Context, ruleEngine typex.Rhilex) {
	uuid, _ := c.GetQuery("uuid")
	mGoods, err := service.GetGoodsWithUUID(uuid)
	if err != nil {
		c.JSON(common.HTTP_OK, common.Error400(err))
