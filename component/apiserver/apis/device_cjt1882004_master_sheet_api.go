...
func InitCjt1882004Route() {
	Api := server.RouteGroup(server.ContextUrl("/cjt1882004_master_sheet"))
	{
		Api.POST(("/sheetImport"), server.AddRoute(Cjt1882004MasterSheetImport))
		Api.GET(("/sheetExport"), server.AddRoute(Cjt1882004MasterPointsExport))
		Api.GET(("/list"), server.AddRoute(Cjt1882004MasterSheetPageList))
		Api.POST(("/update"), server.AddRoute(Cjt1882004MasterSheetUpdate))
		Api.DELETE(("/delIds"), server.AddRoute(Cjt1882004MasterSheetDelete))
		Api.DELETE(("/delAll"), server.AddRoute(Cjt1882004MasterSheetDeleteAll))
	}
}

type Cjt1882004MasterPointVo struct {
...
*  http://127.0.0.1:2580/api/v1/Cjt1882004_data_sheet/export
 */

// Cjt1882004MasterPoints 获取Cjt1882004_excel类型的点位数据
...
func Cjt1882004MasterSheetPageList(c *gin.Context, ruleEngine typex.Rhilex) {
	...
	result := tx.Order("created_at DESC").Find(&records,
		&model.MCjt1882004DataPoint{DeviceUuid: deviceUuid})
	if result.Error != nil {
		c.JSON(common.HTTP_OK, common.Error400(result.Error))
		return
	}
	recordsVo := []Cjt1882004MasterPointVo{}
...
*
 */
func Cjt1882004MasterSheetDeleteAll(c *gin.Context, ruleEngine typex.Rhilex) {
	type Form struct {
	...
	err := service.DeleteAllMCjt1882004ByDevice(form.DeviceUUID)
	if err != nil {
		c.JSON(common.HTTP_OK, common.Error400(err))
		return
	}
...
*
 */
func Cjt1882004MasterSheetDelete(c *gin.Context, ruleEngine typex.Rhilex) {
	type Form struct {
		UUIDs      []string `json:"uuids"`
		DeviceUUID string   `json:"device_uuid"`
	}
	...
	err := service.DeleteCjt1882004PointByDevice(form.UUIDs, form.DeviceUUID)
	if err != nil {
		c.JSON(common.HTTP_OK, common.Error400(err))
		return
	}
...
func CheckCjt1882004MasterDataPoints(M Cjt1882004MasterPointVo) error {
...
func Cjt1882004MasterSheetUpdate(c *gin.Context, ruleEngine typex.Rhilex) {
	...
	for _, Cjt1882004MasterDataPoint := range form.Cjt1882004MasterDataPoints {
		if err := CheckCjt1882004MasterDataPoints(Cjt1882004MasterDataPoint); err != nil {
			c.JSON(common.HTTP_OK, common.Error400(err))
			return
		}
		// "MeterId", "Tag", "Alias", "Frequency"
		if Cjt1882004MasterDataPoint.UUID == "" ||
			Cjt1882004MasterDataPoint.UUID == "new" ||
			Cjt1882004MasterDataPoint.UUID == "copy" {
			NewRow := model.MCjt1882004DataPoint{
				UUID:       utils.Cjt1882004PointUUID(),
				DeviceUuid: Cjt1882004MasterDataPoint.DeviceUuid,
			...
			OldRow := model.MCjt1882004DataPoint{
				UUID:       Cjt1882004MasterDataPoint.UUID,
				DeviceUuid: Cjt1882004MasterDataPoint.DeviceUuid,
				MeterId:    Cjt1882004MasterDataPoint.MeterId,
				Tag:        Cjt1882004MasterDataPoint.Tag,
				Alias:      Cjt1882004MasterDataPoint.Alias,
...
type Cjt1882004DeviceDto struct {
	UUID   string
	Name   string
	Type   string
	Config string
...
func (md Cjt1882004DeviceDto) GetConfig() map[string]interface{} {
...
// Cjt1882004MasterSheetImport 上传Excel文件
func Cjt1882004MasterSheetImport(c *gin.Context, ruleEngine typex.Rhilex) {
	...
	if Device.Type != (string(typex.CJT1882004_MASTER)) {
	...
	list, err := parseCjt1882004MasterPointExcel(file, "Sheet1", deviceUuid)
	if err != nil {
		c.JSON(common.HTTP_OK, common.Error400(err))
		return
	}
	if err = service.InsertCjt1882004Points(list); err != nil {
		c.JSON(common.HTTP_OK, common.Error400(err))
		return
	}
...
func parseCjt1882004MasterPointExcel(r io.Reader, sheetName string,
	deviceUuid string) (list []model.MCjt1882004DataPoint, err error) {
	...
	for i := 1; i < len(rows); i++ {
		...
		//
		model := model.MCjt1882004DataPoint{
			UUID:       utils.Cjt1882004PointUUID(),
