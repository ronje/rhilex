...
func Test_MNIST(t *testing.T) {
	...
	test, err := load1("./data/mnist_test.csv")
	if err != nil {
		panic(err)
	}
	...
	trainer.Train(neural, train, test, 100)
	for i := range test {
		result := neural.Predict(test[i].Input)
		p10(test[i].Response, result)
	}
	bin, err := neural.Marshal()
	if err != nil {
		panic(err)
	}
	os.WriteFile("./data/mnist_model.json", bin, 0755)
}
func p10(Expect []float64, Input []float64) [10]float64 {
	result := [10]float64{}
	for i, v := range Input {
		result[i] = math.Round(v)
	}
	fmt.Println("Result", result, ", Expect", Expect)
	return result
}
func load1(path string) (training.Examples, error) {
	f, err := os.Open(path)
	if err != nil {
		return nil, err
	}
	...
	for {
		...
		examples = append(examples, toExample1(record))
	}

	return examples, nil
}

func toExample1(in []string) training.Example {
	res, err := strconv.ParseFloat(in[0], 64)
	if err != nil {
		panic(err)
	}
	resEncoded := onehot1(10, res)
...
func onehot1(classes int, val float64) []float64 {
