...
func TestBinaryMatch(t *testing.T) {
	...
	t.Log("rhilexlib.Match ==> <a:1 b:3 b:5", "000011111010101000001010")
	kls := rhilexlib.Match("<a:1 b:3 b:5", []byte{0b00001111, 0b10101010, 0b00001010}, true)
	var len uint = 0
	for _, v := range kls {
		len += v.L
	}
...
func TestBinaryMatch_big(t *testing.T) {
	...
	kls := rhilexlib.Match(">a:8 b:8 c:8", []byte("aab"), true)
	var len uint = 0
	for _, v := range kls {
		len += v.L
...
func TestBinaryMatch_little(t *testing.T) {
	...
	kls := rhilexlib.Match("<k1:8 k2:8 k3:8", []byte("aab"), true)
	var len uint = 0
	for _, v := range kls {
		len += v.L
		t.Log("字段:", v.K, " 二进制串:", v.BS)
