...
import (
	...
	amlogics805 "github.com/hootrhino/rhilex/archsupport/amlogics805"
...
func WKYWS1608_GPIOGet(rx typex.Rhilex, uuid string) func(*lua.LState) int {
	return func(l *lua.LState) int {
		...
		v, e := amlogics805.AmlogicWKYS805_RGBGet(pin)
		if e != nil {
			l.Push(lua.LNil)
...
func WKYWS1608_GPIOSet(rx typex.Rhilex, uuid string) func(*lua.LState) int {
	return func(l *lua.LState) int {
		...
		_, e := amlogics805.AmlogicWKYS805_RGBSet((pin), int(value))
		if e != nil {
			l.Push(lua.LString(e.Error()))
		} else {
			l.Push(lua.LNil)
